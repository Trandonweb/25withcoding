...
import { db, getDocs, collection, query, orderBy, deleteDoc, doc, updateDoc, serverTimestamp } from "./firebase.js";
...
export async function loadAdminLogs() {
    ...
    try {
        ...
        for (const user of users.docs) {
            const uid = user.id, userData = user.data();
        ...
        console.error("관리자 대화 로그 로드 실패:", e);
...
function renderUserConversations(group, list) {
    ...
    back.innerHTML = `<button type="button">‹ 사용자 목록</button><strong>${escapeHtml(group.name)}의 대화</strong>`;
    ...
    const render = tab => {
        ...
        const rows = tab === "trash" ? group.rows.filter(r => r.data.isDeleted === true || r.data.hiddenFromUser === true) : group.rows.filter(r => !r.data.isDeleted && !r.data.hiddenFromUser);
        content.innerHTML = "";
        if (!rows.length) {
...
function renderConversation(row, group, content, tab, refresh) {
    ...
    // 헤더 옆에 상태별 관리 버튼을 항상 노출한다. 대화를 펼칠 필요가 없다.
    const head = document.createElement("div");
    head.className = "admin-log-row-head";
    head.innerHTML = `<div class="admin-log-row-title"><strong>${escapeHtml(row.data.title || "새 대화")}</strong><small>${formatDate(row.data.updatedAt)} · ${status} · ${row.messages.length}개 메시지</small></div>`;
    ...
    if (tab === "trash") {
        ...
        button.onclick = async event => {
            ...
            if (isHiddenFromUser) {
                ...
                button.disabled = true; button.textContent = "DB 삭제 중...";
                try {
                    ...
                    if (!group.rows.some(x => x.data.isDeleted === true || x.data.hiddenFromUser === true)) refresh("trash");
                } catch (error) {
            ...
            try {
                await updateDoc(doc(db, "people", row.uid, "conversations", row.cid), { hiddenFromUser: true, userPurgedAt: serverTimestamp(), userPurgedBy: state.currentUserId, updatedAt: serverTimestamp() });
                row.data.hiddenFromUser = true;
                button.className = "admin-db-delete"; button.textContent = "DB 삭제"; button.disabled = false;
            } catch (error) {
                console.error("관리자 영구 삭제 처리 실패:", error); alert("영구 삭제 처리에 실패했습니다.");
                button.disabled = false; button.textContent = "영구 삭제";
            }
        };
    ...
    head.addEventListener("click", event => { if (!event.target.closest("button")) body.hidden = !body.hidden; });
