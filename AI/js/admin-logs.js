        ...
        if (row.data.hiddenFromUser) {
            ...
            button.onclick = async e => {
                ...
                try {
                    ...
                    element.remove(); await loadChats(); await loadTrash();
                    if (!group.rows.some(x => x.data.isDeleted)) content.innerHTML = '<div class="settings-empty">휴지통이 비어 있습니다.</div>';
                } catch (error) {
            ...
            button.onclick = async e => {
                ...
                try {
                    ...
                    button.onclick = () => renderUserConversations(group, listFor(content));
...
// 현재 탭 컨테이너를 다시 찾기 위한 안전한 보조 함수
function listFor(content) { return content.parentElement; }

export async function openAdminLogs() {
    if (!isAdmin()) return alert("관리자 권한이 필요합니다.");
    closeMenu();
    const modal = $("adminLogsModal"); if (!modal) return console.error("adminLogsModal 요소를 찾을 수 없습니다.");
    modal.classList.add("show"); modal.setAttribute("aria-hidden", "false"); await loadAdminLogs();
}
