...
export async function loadChats() {
    ...
    if (!docs.length) {
        $("chatList").innerHTML = '<div class="empty-chat">아직 대화가 없습니다.</div>';
        return;
    }
    ...
    docs.forEach(item => {
        ...
        element.querySelector(".chat-delete").onclick = async event => {
            ...
            if (confirm("이 대화를 휴지통으로 이동할까요?")) await moveToTrash(item.id);
        };
...
export async function createConversation(projectId = null) {
    const result = await addDoc(people(), {
        title: "새 대화",
...
export async function deleteConversationFromDb(uid, id) {
    ...
    try {
        const snapshot = await getDocs(collection(db, "people", uid, "conversations", id, "messages"));
        await Promise.all(snapshot.docs.map(message => deleteDoc(message.ref)));
        await deleteDoc(doc(db, "people", uid, "conversations", id));
        return true;
    } catch (error) {
        console.error(error);
        return false;
